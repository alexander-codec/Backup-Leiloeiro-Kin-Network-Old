$("#paghome").removeClass("active");$("#pagagenda").removeClass("active");$("#paglances").removeClass("active");$("#pagmodalidade").addClass("active");$("#pagcategoria").removeClass("active");$("#pagcontato").removeClass("active");
$(window).scrollTop(0);
load();
...
function load() {
	...
	if($.urlParam('id') == null){
		...
		$("#listleiloes").html("<div class='col-md-12'><div class='error-content'><div class='error-message'><h2>404</h2><h3><span>Ooooops!</span> Nenhum Leilão Encontrado Nessa Modalidade...</h3></div></div></div>");
	}else{
		var modalidade = $.urlParam('id');
		doctRef.get().then(function(doct) {
			if (doct.exists) {
				var post = 0;
				doctRef.collection("leiloes").where("modalidade", "==", modalidade)//.limit(24)
		            ...
		            querySnapshot.forEach(function(doc) {
											...
											if(post == 1){
	                                                $("#showcont").text("Mostrando (" + post + " Leilão) da Modalidade (" + modalidade + ")");
	                                        }else if(post > 1){
	                                                $("#showcont").text("Mostrando (" + post + " Leilões) da Modalidade (" + modalidade + ")");
	                                        }
					});
					if(post == "0"){
						$("#showcont").text("Nenhum Leilão para Mostrar");
