$("#paghome").removeClass("active");$("#pagagenda").removeClass("active");$("#paglances").removeClass("active");$("#pagmodalidade").removeClass("active");$("#pagcategoria").addClass("active");$("#pagcontato").removeClass("active");
$(window).scrollTop(0);
load();
...
function load() {
	...
	if($.urlParam('id') == null){
		...
		$("#listleiloes").html("<div class='col-md-12'><div class='error-content'><div class='error-message'><h2>404</h2><h3><span>Ooooops!</span> Nenhum Leilão Encontrado Nessa Categoria...</h3></div></div></div>");
	}else{
		var cat = $.urlParam('id');
		doctRef.get().then(function(doct) {
			if (doct.exists) {
				var post = 0;
				doctRef.collection("leiloes").where("categoria", "==", cat)//.limit(24)
		            ...
		            querySnapshot.forEach(function(doc) {
											...
											if(post == 1){
	                                                $("#showcont").text("Mostrando (" + post + " Leilão) da Categoria (" + categoria + ")");
	                                        }else if(post > 1){
	                                                $("#showcont").text("Mostrando (" + post + " Leilões) da Categoria (" + categoria + ")");
	                                        }
					});
					if(post == "0"){
						$("#showcont").text("Nenhum Leilão para Mostrar");
