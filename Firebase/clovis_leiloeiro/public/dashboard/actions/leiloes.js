$("#paghome").removeClass("active");$("#pagleiloes").addClass("active");$("#pagpreferencias").removeClass("active");$("#paglances").removeClass("active");$("#pagestatisticas").removeClass("active");
var postcont = 0;
...
doctRef.collection("leiloes").orderBy("update", "desc")
		            .onSnapshot(function(querySnapshot) {
		                ...
		                querySnapshot.forEach(function(doc) {
		                    ...
		                    $("#listleiloes").append("<tr><td>" + doc.data().titulo + "</td><td>" + doc.data().cidade + ", " + doc.data().estado + "</td><th>" + status + "</th><td>" + update + "</td><td><div class='dropdown'><button class='btn btn-secondary dropdown-toggle' type='button' id='" + doc.id + "' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>Menu</button><div class='dropdown-menu' aria-labelledby='" + doc.id + "'><a class='dropdown-item' href='/dashboard/#/editar?id=" + doc.id + "&modalidade=" + doc.data().modalidade + "'>Editar</a><a class='dropdown-item' href='/#/leilao?id=" + doc.id + "' target='_blank'>Visualizar</a><a class='dropdown-item' imgs='" + doc.data().imgs + "' id='" + doc.id + "' href='javascript:void(0)' onclick='excluir(this);'>Excluir</a></div></div></td></tr>");
		                    if(postcont == "1"){
		                        $("#notshow").hide();
		                    }
		                });
